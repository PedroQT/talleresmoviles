  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Gestiona las universidades en tiempo real'),
      ...
      body: StreamBuilder<List<UniversidadFb>>(
        stream: UniversidadesService.watchUniversidades(),
...
class _UniversidadCard extends StatelessWidget {
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                universidad.direccion.isEmpty ? 'Sin dirección' : universidad.direccion,
                style: TextStyle(
                  fontSize: 13,
                  color: universidad.direccion.isEmpty
                      ? colorScheme.onSurfaceVariant.withValues(alpha: 0.5)
                      : colorScheme.onSurfaceVariant,
                  fontStyle: universidad.direccion.isEmpty ? FontStyle.italic : FontStyle.normal,
                ),
                maxLines: 2,
  ...
  Future<void> _showDeleteDialog(BuildContext context) async {
    ...
    if (confirmar == true && context.mounted) {
      try {
        await UniversidadesService.deleteUniversidad(universidad.id);
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
